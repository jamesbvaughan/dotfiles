...
def getActivePlayer():
    ...
    playing = [player['name']
               for player in players if player['status'] == 'Playing']
    paused = [player['name']
              for player in players if player['status'] == 'Paused']
    if len(playing):
        return playing[-1]
    if len(paused):
        return paused[-1]
...
class PlayerStatus:
    ...
    def _print_song(self):
        label = '{} {} - {}'.format(self._icon, self._title, self._artist)
        self._print_flush(label)
